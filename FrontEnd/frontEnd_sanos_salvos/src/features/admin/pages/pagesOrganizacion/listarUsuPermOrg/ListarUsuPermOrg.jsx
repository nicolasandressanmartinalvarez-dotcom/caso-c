...
function ListarUsuPermOrg() {
    ...
    const cargarUsuarios = async () => {
        try {
            ...
            const response = await fetch("http://localhost:8089/api/usuPermitidosOrg", {
        ...
        <section className={ListarUsuPermCSS["contenedor-usuarios"]}>
            <div className={ListarUsuPermCSS["header-lista"]}>
                <div className={ListarUsuPermCSS["titulo-seccion"]}>
                    <h2>Usuarios Permitidos Org <FaUserShield className={ListarUsuPermCSS["icono-titulo"]}/></h2>
                    <p>Gestión de accesos y roles de los miembros de organizaciones.</p>
                </div>
            </div>
                    ...
                    <tbody>
                        {filtrados.map((user) => (
                            <tr key={user.id}>
                                ...
                                <td>{user.organizacion?.nombre || "N/A"}</td>
                                <td>
                                ...
                                <td className={ListarUsuPermCSS["acciones-cell"]}>
                                    <button className={ListarUsuPermCSS["btn-icon"]} title="Editar permisos" onClick={() => navigate(`/admin/editarUserPermOrg/${user.id}`)}>
                                        <FaUserEdit />
                                    </button>
...
export default ListarUsuPermOrg;
